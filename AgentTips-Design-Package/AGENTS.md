...
- 禁止为了测试 AgentTips 修改 Codex runtime、Computer Use runtime、IDE runtime、
  Windows Shell、Tauri 安装目录或其他第三方应用文件。
- OS 原生交互（系统托盘菜单、原生标题栏等）无法稳定自动化时，保留人工冒烟测试，
  不得通过修改外部运行时换取"100% 自动化"。
- 测试数据隔离只允许通过应用自身提供的测试路径（`AGENTTIPS_TEST_DATA_DIR`，
  仅 debug/test 构建读取），不得依赖修改系统环境或第三方文件。
