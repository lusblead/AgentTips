...
| 术语         | 定义                                                        |
| ------------ | ----------------------------------------------------------- |
| Tip / 便签   | 用户记录的一段无标题文本，可添加自定义标签并绑定多个 Agent  |
...
4. 新便签默认无标题，支持正文、用户自定义标签以及零个或多个 Agent 绑定；用户可以先记录、之后再补标签或绑定。
...
- 图片、附件、富文本协作；
- macOS、Linux、移动端；
- AI 生成标签、摘要或提示词优化；标签只由用户输入或从自己的历史标签中复用。
...
- 自动聚焦正文编辑区。
- Agent 绑定可选；当前检测到 Agent 时可预选该 Agent，但用户不绑定也能保存。
- 快速窗口不提供标题输入，保存后标题保持为空，不从正文派生标题。
- 标签由用户自由输入；输入时可以看到自己以前写过的匹配标签并点击复用，但不存在固定标签选项或自动生成标签。
- `Ctrl + Enter` 保存；`Esc` 请求关闭。
- 空白便签关闭时不保存。
- 非空未保存内容通过 `Esc`、界面关闭按钮或系统标题栏关闭时，统一显示“继续编辑 / 放弃内容”确认；没有明确确认不得清空或隐藏。
